...
export async function createRosProject() {
    ...
    vscode.workspace.fs.writeFile(vscode.Uri.joinPath(directory, 'CMakeLists.txt'), Buffer.from(project.generateCMakeLists()));
    vscode.workspace.fs.writeFile(vscode.Uri.joinPath(directory, 'package.xml'), Buffer.from(project.generatePackageXml()));
...
export async function addMsgToProject(project?: RosProject, messages?: vscode.Uri[]) {
    ...
    if (!messages) {
        let selectedMessage = await vscode.window.showOpenDialog({
            ...
            canSelectFiles: true,
            canSelectMany: true,
            defaultUri: project.rootDirectory
        });
